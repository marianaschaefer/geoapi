...
PRINT_VERSION_TAG = "seg-v4-stac-alias"
...
# Possíveis aliases por banda (nomes alternativos que aparecem em assets STAC)
...
def _match_asset_key(item, band_id: str) -> Optional[str]:
    ...
    - chave exata: 'B04'
    - com sufixo de resolução: 'B04_10m', etc.
    - qualquer chave que contenha 'B04'
    - por 'name'/'common_name' em eo:bands/raster:bands
    """
    ...
    # 2) chave que contenha 'Bxx'
    for k, a in item.assets.items():
        if band_id in k:
            return a.href

    # 3) metadados (eo:bands / raster:bands)
    ...
    for coll in collections_order:
        ...
        for it in items:
            if _match_asset_key(it, 'B04'):
                print(f"[INFO] Item escolhido da coleção '{coll}', cloud_cover=",
...
def baixar_bandas_earthsearch(output_dir=OUTPUT_DIR, dias=180, resolucao=10, bbox=None, max_cloud=80):
    ...
    width, height = bbox_to_dimensions(aoi_sh, resolution=resolucao)  # estima tamanho
    ...
    for banda in BANDAS_TODAS:
        ...
        try:
            with COGReader(href) as cog:
                arr, mask = cog.part(aoi, bounds_crs="EPSG:4326", max_size=(width, height))
            data = arr[0].astype("uint16")  # (H,W)
            if mask is not None:
